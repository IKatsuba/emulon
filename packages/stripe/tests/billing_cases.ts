...
async function rejects(
  ...
  try {
    ...
    assert(
      error instanceof Stripe.errors.StripeError && check(error),
...
async function started(
  ...
  const env = await Emulon.start({ services: { stripe: plugin(options) } });
  const { apiKey } = await env.services.stripe.keys.create({});

  return {
    env,
    clock,
    sdk: client(env.endpoints.stripe.api!, apiKey),
...
register(
  ...
  async () => {
    ...
    assert(product.tax_code === 'txcd_10103001' && product.type === 'service');
...
register(
  ...
  async () => {
    ...
    const code = await sdk.promotionCodes.create({
    ...
    const expanded = code.promotion.coupon as Stripe.Coupon;
    ...
    assert(
      (await sdk.promotionCodes.retrieve(code.id)).promotion.coupon ===
        coupon.id,
    );
    await rejects(
      () =>
        sdk.promotionCodes.create({
          promotion: { type: 'coupon', coupon: fixed.id },
          code: 'launch99',
        }),
      invalid('code'),
    );
    ...
    await rejects(
      () =>
        sdk.promotionCodes.create({
          promotion: { type: 'coupon', coupon: 'NOPE' },
        }),
      (e) => e.code === 'resource_missing' && e.param === 'promotion[coupon]',
    );

    const generated = await sdk.promotionCodes.create({
      promotion: { type: 'coupon', coupon: fixed.id },
    });
    ...
    assert(listed.data.every((p) => typeof p.promotion.coupon === 'object'));
    assert(
    ...
    assert(!off.active && typeof off.promotion.coupon === 'object');
    assert(
      (await sdk.promotionCodes.list({ code: 'LAUNCH99', active: true })).data
        .length === 0,
    );
    assert((await sdk.promotionCodes.update(code.id, { active: true })).active);
...
register(
  ...
  async () => {
    ...
    assert(session.amount_subtotal === 12900 && session.amount_total === 9675);
    ...
    await sdk.promotionCodes.create({
      promotion: { type: 'coupon', coupon: full.id },
      code: 'FREE',
    });
    ...
    assert(
      free.amount_total === 0 && free.payment_status === 'no_payment_required',
    );
    ...
    assert(
      (await fetch(lapsing.url!).then((r) => r.text())).includes('is expired'),
    );
    ...
    assert(types.filter((t) => t === 'checkout.session.expired').length === 2);
    await rejects(
...
register(
  ...
  async () => {
    ...
    const config = { services: { stripe: stripe() } };
    ...
    try {
      ...
      const pay = async (email: string) => {
        ...
        const done = await cli('checkout', 'sessions', 'complete', session.id);
      ...
      assert(
        (await cli('disputes', 'create', '--charge', second.charge)).code !== 0,
      );
      ...
      const refundedEvents = events.filter((e) => e.type === 'charge.refunded');
...
register(
  ...
  async () => {
    ...
    const verifier = new Stripe('sk_test_unused', { apiVersion: version });
