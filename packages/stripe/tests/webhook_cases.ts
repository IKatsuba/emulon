...
import { dahliaFlavor, type Flavor } from './flavors.ts';
...
const modules = new Map([[dahlia.id, dahlia], [basil.id, basil]]);

/** The suites a flavor runs its webhook contract through. */
export function webhookCases(flavor: Flavor, register: Register) {
  ...
  async function retryContract() {
    ...
    const receiver = Deno.serve(
      ...
      async (request) => {
        ...
        const parsed = await flavor.Stripe.webhooks.constructEventAsync(
          body,
          signature,
          currentSecret,
          300,
    ...
    const worker = deliveryWorker(
      store,
      transport(new Map([[module.id, module]])),
      timer,
    );
    ...
    try {
      ...
      for (const [index, delay] of [60000, 3600000, 7200000].entries()) {
        ...
        assert(
          Date.parse(inspection.delivery.nextAttemptAt!) ===
            timer.now() + delay,
        );
  ...
  async function commandContract() {
    ...
    const receiver = Deno.serve(
      ...
      async (request) => {
        ...
        await flavor.Stripe.webhooks.constructEventAsync(
          body,
    ...
    try {
      ...
      assert(
        (await service.webhooks.inspect({ id: direct.id })).attempts.length ===
          2,
      );
...
webhookCases(dahliaFlavor, register);
