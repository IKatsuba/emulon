...
import { z } from 'zod';
import { type CustomerCreatedPayload, payloadSchema } from '../webhooks/mod.ts';

type Operation<Input, Output> = ReturnType<
  typeof defineCommand<z.ZodType<Input, Input>, z.ZodType<Output>>
>;
type PublishInput = { type: 'customer.created'; data: CustomerCreatedPayload };
...
export const commands: Commands = {
  'webhooks.send': defineCommand({
    description:
      'Send a direct webhook without creating a customer or selecting subscriptions',
  ...
  'events.publish': defineCommand({
    description:
      'Publish a synthetic customer event without creating a customer',
    input: z.strictObject({
      type: z.literal('customer.created'),
      data: payloadSchema,
    }),
