...
single-organization instance accepts only its configured organization ID. Apply
ADR 0033's fixed `Polar-Version: 2026-04` selection to these routes
independently of OAT authentication; missing selects the pin, and an unsupported
supplied version still fails before mutation. The
...
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `benefits.create`        | `benefits create --description [--prefix] [--limit-activations --enable-customer-admin] [--ttl --timeframe] [--limit-usage]` | `{ description, properties: { prefix?, activations?: { limit, enableCustomerAdmin }, expires?: { ttl, timeframe }, limitUsage? } }` creates `BenefitLicenseKeys`. `type` is fixed to `license_keys` and the organization comes from the instance. Paired fields must be supplied together. |
